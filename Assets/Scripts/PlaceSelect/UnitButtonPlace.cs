...
public class UnitButtonPlace :
    MonoBehaviour,
    IPointerEnterHandler,
    IPointerExitHandler
{
    private GameManager gameManager;             // マネージャコンポ
    private PlaceSelect placeCommon;             // Placeシーンコントローラ
    public int unitID = 0;                       // ユニットID（InstantiateするPlaceSelectクラスから設定される）
    ...
    void Start()
    {
        ...
        // アビリティシーンコントローラ取得
        placeCommon = GameObject.FindWithTag("Canvas").GetComponent<PlaceSelect>();
    ...
    public IEnumerator MouseClickHandler()
    {
        ...
        while (1 == mouseOverJug)
        {
            ...
            if (Input.GetMouseButtonDown(0) && Defines.ABL_NON_VALUE == placeCommon.unitSelect)
            {
                // クリックSEを鳴らす
                audioCompo.Play();
                ...
                placeCommon.unitSelect = unitID;
            }
            // マウス右クリックされ、かつ本ユニット選択済みの場合
            else if (Input.GetMouseButtonDown(1) && Defines.ABL_NON_VALUE != placeCommon.unitSelect)
            {
                // クリックSEを鳴らす
                audioCompo.Play();
