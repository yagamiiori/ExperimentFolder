...
public class ComboBoxAttribute : MonoBehaviour
{
    ...
    /// <summary>プルダウンメニューで選択されたエレメントのID</summary>
    public int SelectedElement
    {
        ...
        set
        {
            ...
            if (value > -1 && value < ElementList.Length)
            {
                // 選択されたIDが正常範囲内であればフィールドに設定する
                _selectedElement = value;

                // エレメントID設定メソッドをコールし、選択されたエレメントIDを設定する
                SetElementIDtoManager(value);
                RefreshSelected();
            }
        }
    }
    ...
    [SerializeField]
    private ComboBoxItem[] _elementList;
    /// <summary>プルダウンメニューを開いた時に表示されるアイテムの総数</summary>
    public ComboBoxItem[] ElementList
    {
        get
        {
            if (_elementList == null)
                // 存在しなければ空の配列を生成する
                _elementList = new ComboBoxItem[0];
            return _elementList;
        }
        set
        {
    ...
    /// <para>　そのユニットIDのエレメントフィールドへ選択されたエレメントIDを設定する。</para>
    /// </summary>
    private void SetElementIDtoManager(int setterValue)
    {
        ...
        // 設定されたエレメントIDをマネージャーコンポに設定
        gameManager.unitStateList[unitID - 1].element = setterValue + 1;
    }

    public void OnItemClicked(int index)
    {
        var selectionChanged = index != SelectedElement;
    ...
    public void AddItems(params object[] list)
    {
        ...
        var newItems = new ComboBoxItem[ElementList.Length + cbItems.Count];
        ElementList.CopyTo(newItems, 0);
        cbItems.ToArray().CopyTo(newItems, ElementList.Length);
        Refresh();
    ...
    {
        ElementList = new ComboBoxItem[0];
    }
    ...
    public void InitControl()
    {
        ...
        var dropdownHeight = comboButtonRectTransform.sizeDelta.y * Mathf.Min(ItemsToDisplay, ElementList.Length - (HideFirstItem ? 1 : 0));
        ...
        // スクロールバーの横幅を決定？
        var scrollbarWidth = ElementList.Length - (HideFirstItem ? 1 : 0) > _itemsToDisplay ? _scrollbarWidth : 0.0f;
    ...
    public void Refresh()
    {
        ...
        var itemsLength = ElementList.Length - (HideFirstItem ? 1 : 0);
        ...
        for (var i = 0; i < ElementList.Length; i++)
        {
            ...
            var item = ElementList[i];    // フィールド載せ替え
    ...
    public void RefreshSelected()
    {
        ...
        var item = SelectedElement > -1 && SelectedElement < ElementList.Length ? ElementList[SelectedElement] : null;
        ...
        foreach (Transform child in itemsRectTransfrom)
        {
            ...
            comboButtonImage.color = SelectedElement == i + (HideFirstItem ? 1 : 0) ? comboButtonButton.colors.highlightedColor : comboButtonButton.colors.normalColor;
            i++;
        }
    }
    ...
    {
        var selectedIndex = SelectedElement + (HideFirstItem ? 1 : 0);
        if (selectedIndex < scrollOffset)
            scrollOffset = selectedIndex;
        else
            if (selectedIndex > scrollOffset + ItemsToDisplay - 1)
                scrollOffset = selectedIndex - ItemsToDisplay + 1;
        var itemsCount = ElementList.Length - (HideFirstItem ? 1 : 0);
    ...
    public void UpdateGraphics()
    {
        if (overlayGO != null)
        {
            var scrollbarWidth = ElementList.Length - (HideFirstItem ? 1 : 0) > ItemsToDisplay ? _scrollbarWidth : 0.0f;
        ...
        if (rectTransform.sizeDelta != buttonRectTransform.sizeDelta && buttonRectTransform.sizeDelta == comboButtonRectTransform.sizeDelta)
        {
            ...
            UpdateComboBoxImage(comboButtonRectTransform, ElementList[SelectedElement].Image != null);
            Refresh();
        }
    }
}